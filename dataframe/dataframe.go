...
// Show prints the DataFrame
func (df DataFrame) Show(rows int) {
	fmt.Println(df.print(true, false, true, true, rows, 70, "DataFrame"))
}
...
type Agg func(DataFrame) interface{}

func NewAgg(colname string, f series.AggregationFunc) Agg {
	return func(df DataFrame) interface{} {
		return f(df.Col(colname))
	}
}

// Agg :Aggregate dataframe by aggregation functions given new column names
func (gps Groups) Agg(colnames []string, aggs ...Agg) DataFrame {
	if gps.groups == nil {
		return DataFrame{Err: fmt.Errorf("Aggregation: input is nil")}
	}
	if len(aggs) != len(colnames) {
		return DataFrame{Err: fmt.Errorf("Aggregation: len(aggs) != len(colanmes)")}
	}
	dfMaps := make([]map[string]interface{}, 0)
	for _, df := range gps.groups {
		targetMap := df.Maps()[0]
